 ...
 *
 * @ViewsFilter("donation_origin")
 */
class DonationOrigin extends InOperator implements ContainerFactoryPluginInterface {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    ...
    return new static($configuration, $plugin_id, $plugin_definition, $container->get('language_manager'), $entity_type_manager->getStorage($configuration['donation_target_entity_type_id']), $container->get('entity_type.bundle.info'), $entity_type_manager->getDefinition($configuration['donation_target_entity_type_id']));
  }
  ...
  protected function defineOptions() {
    ...
    $options['donation_target_bundles'] = [
      'default' => [],
    ];
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    if ($this->targetEntityType->hasKey('bundle')) {
      ...
      $form['donation_target_bundles'] = [
        '#type' => 'checkboxes',
        '#title' => $this->t('Target entity bundles to filter by'),
        '#options' => $options,
        '#default_value' => array_filter($this->options['donation_target_bundles']),
      ];
    }
  ...
  public function getValueOptions() {
    ...
    foreach ($this->getReferenceableEntities() as $entity) {
      ...
      $in_donation = \Drupal::entityQuery('donation')
        ->condition('origin', $entity->id())
        ->execute();
  ...
  protected function getReferenceableEntities() {
    ...
    $target_bundles = array_filter($this->options['donation_target_bundles']);
